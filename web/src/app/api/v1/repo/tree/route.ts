...
const EXCLUDED_DIRECTORIES = [
    'node_modules', 'vendor', 'dist', 'build', 'out', '.next', '.svelte-kit', 
    '.git', '.vscode', 'public', 'assets', 'static'
];

const EXCLUDED_PATTERNS = [
    /package-lock\.json$/, /yarn\.lock$/, /pnpm-lock\.yaml$/, /\.env$/, /\.log$/,
    /\.gitignore$/, /\.prettierrc$/, /\.eslintrc/, /tailwind\.config/,
    /\.md$/, /\.test\.[jt]sx?$/, /\.spec\.[jt]sx?$/, /__tests__/,
    /\.(jpg|jpeg|png|gif|svg|ico|webp)$/ 
];

const INCLUDED_EXTENSIONS = [
    '.js', '.jsx', '.ts', '.tsx', '.py', '.go', '.java', '.rb', '.php', '.html',
    '.css', '.scss', '.vue', '.svelte', '.c', '.cpp', '.cs', '.json'
];
...
export const POST = async (req: Request, res: Response) => {
    ...
    try {
       ...
       const { data: treeData } = await octokit.rest.git.getTree({ owner, repo, tree_sha: treeSha, recursive: "true" });
